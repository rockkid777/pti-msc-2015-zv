...
3. Pragmatika
    + olvasható, konvencionális, hatékony kód
...
3. **Denotációs (leíró) szemantikával**
    ...
    + Absztraktabb operációs szemantikánál
...
* `"4" == "4"`
    ...
    + **DE!** Nem azt csinálja ami szeretnénk! Nem egyértelmű!
...
* *p* produkciós szabály bal oldalán: *A* nem terminális áll
...
* Ha *p* egy olyan $p = A \rightarrow \alpha$ szabály
    ...
    + akkor az *A.Attr* egy olyan *p* szabályban van $\rightarrow$ ahol *A* a **bal oldalon** van.
* Az attribútumfában felfelé szállít infót (gyerekből szülőbe)

**Örökölt**

* Ha *p* egy olyan $p = A \rightarrow \alpha \times \beta$ szabály
    + ahol $r \in R(p)$ attribútumszámítási szabály beállítja *X.Attr*-t
    + akkor az *X.Attr* egy olyan *p* szabályban van $\rightarrow$ ahol *X* a **jobb oldalon** van.
...
## 5. Alapvető imperatív nyelvi elemek strukturális és természetes műveletei, illetve leíró szemantikája, hasonlóságok és különbségek <a name="chapter05"></a>
...
* Köztes lépés a számításban: $\langle a, s \rangle \Longrightarrow \langle a', s \rangle$
* Részkifejezés kiértékelése a végkonfigurációban: $$\langle a, s \rangle \Longrightarrow v\ where\ v\ in\
  \mathbb{Z} \cup \{tt, ff\}$$
...
$S_1$ és $S_2$ szemantikusan ekvivalensek, ha minden *s* állapotra

* $\langle S_1, s \rangle \Longrightarrow ^* c$ akkor és csak akkor $\langle S_1, s \rangle \Longrightarrow ^* c$
...
\cline{1-2} && B[\![b]\!]s = tt \\
& \langle \textbf{while}\ b\ \textbf{do}\ S, s \rangle \rightarrow \langle s''
\end{align*}
...
S_{ds}[\![if\ b\ then\ S_1\ else\ S_2]\!] = cond(B[\![b]\!], S_{ds}[\![S_1]\!], S_{ds}[\![S_2]\!]) \\
...
2. **A szemantika konzisztens**
    + Ha egy kifejezéshez két különböző jelentés is levezethető egy kifejezéshez $\Longrightarrow$ akkor azok
      ekvivalensek
...
* *F g* akkor eygenlő *g*-vel, ha *g* a fixpont
